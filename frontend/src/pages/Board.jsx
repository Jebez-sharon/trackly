    ...
    return(
        <tr onClick={() => onOpen(issue.id)}
            ...
            <td className="hidden px-4 py-3 align-top sm:table-cell">
                <span className={`inline-flex items-center gap-1.5 whitespace-nowrap text-xs font-medium ${p.text}`}>
                    <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${p.dot}`} aria-hidden="true"/>
                        {p.label}
                </span>
            </td>
            <td className="px-4 py-3 align-top">
                <span className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-md px-2 py-0.5 text-xs font-medium ${s.soft} ${s.text}`}>
                ...
                {current && (
                    ...
                    <div className="mb-4 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        ...
                        <span className="text-[13px] text-ink-soft">
                            {current.issue_count} issue{current.issue_count === 1 ? "" : "s"}
                        </span>
                    </div>
                    ...
                    {!issues.loading && !issues.error && issues.data && issues.data.length === 0 &&(
                        <Message >Nothing has been reported in {current.name} yet.</Message>
                    )}
                        ...
                        <div className="overflow-x-auto rounded-xl border border-line bg-surface">
                            <table className="w-full min-w-[560px] border-collapse">
                                <thead className="border-b border-line">
                                    <tr>
                                        <th scope="col" className="w-24 px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-muted">
                                            Key
                                        </th>
                                        <th scope="col" className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-muted">
                                            Title
                                        </th>
                                        <th scope="col" className="hidden w-32 px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-muted sm:table-cell">
                                            Priority
                                        </th>
                                        <th scope="col" className="w-36 px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-muted">
                                            Status
                                        </th>
                                        <th scope="col" className="hidden w-40 px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-muted md:table-cell">
                                            Assignee
                                        </th>
                                    </tr>
                                </thead>
